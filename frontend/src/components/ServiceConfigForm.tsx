...
import {
  ServiceCardSelector,
  ServiceOption,
} from "src/components/ServiceCardSelector";
...
const fallbackT: TFunction = (
  ...
  const map: Record<string, string> = {
    ...
    "services.validation.explorerRequired":
      "Flokicoin Explorer URL is required.",
    "services.validation.swapRequired":
      "Swap Service URL is required when enabled.",
    "services.validation.messageboardRequired":
      "Messageboard URL is required when enabled.",
...
export const validateServiceConfig = (
  ...
  if (!state.relay) {
    ...
    for (const relayUrl of relays) {
      ...
      if (relayErr) {
        errors.push(
          t("services.validation.invalidUrl", { field: "Nostr Relay URL" })
        );
        break;
      }
    }
  ...
  const generalRelays = state.generalRelay
    .split(",")
    .map((r) => r.trim())
    .filter((r) => r.length > 0);
  for (const relayUrl of generalRelays) {
    ...
    if (generalRelayErr) {
      errors.push(
        t("services.validation.invalidUrl", { field: "General Relay URL" })
      );
      break;
    }
  }

  const searchRelays = state.searchRelay
    .split(",")
    .map((r) => r.trim())
    .filter((r) => r.length > 0);
  for (const relayUrl of searchRelays) {
    ...
    if (searchRelayErr) {
      errors.push(
        t("services.validation.invalidUrl", { field: "Search Relay URL" })
      );
      break;
    }
  }
  ...
  if (!state.mempoolApi) {
    ...
    const mempoolErr = validateHTTPURL(
      state.mempoolApi,
      "Flokicoin Explorer URL"
    );
    if (mempoolErr)
      errors.push(
        t("services.validation.invalidUrl", { field: "Flokicoin Explorer URL" })
      );
  }
  ...
  if (state.enableSwap) {
    if (!state.swapServiceUrl) {
      ...
      if (swapErr)
        errors.push(
          t("services.validation.invalidUrl", { field: "Swap Service URL" })
        );
    }
  }
  ...
  if (state.enableMessageboardNwc) {
    if (!state.messageboardNwcUrl) {
      ...
      if (mbErr)
        errors.push(
          t("services.validation.invalidUrl", { field: "Messageboard URL" })
        );
    }
  }
...
export function ServiceConfigForm({
  ...
  className,
  validationErrors = [],
  showGeneralRelay = true,
  showSearchRelay = true,
  ...
  const relayTabs = [
    ...
    ...(showGeneralRelay
      ? [
          {
            key: "general",
            label: t("services.generalRelay.tabLabel"),
            icon: Users,
          },
        ]
      : []),
    ...(showSearchRelay
      ? [
          {
            key: "search",
            label: t("services.searchRelay.tabLabel"),
            icon: Search,
          },
        ]
      : []),
    ...
    <div className={`space-y-6 ${className || ""}`}>
            ...
            <Tabs defaultValue={relayTabs[0].key}>
                ...
                <div className="flex items-start justify-between gap-4 rounded-lg border border-border p-3">
                  <div className="space-y-1">
                    <p className="text-sm font-medium">
                      {t("services.trustedNwcRelay.title")}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {t("services.trustedNwcRelay.description")}
                    </p>
                  </div>
                  <Switch
                    checked={state.trustedNwcRelay}
                    onCheckedChange={(checked) =>
                      onChange({ ...state, trustedNwcRelay: checked })
                    }
                  />
                </div>
              </TabsContent>
              ...
              {showGeneralRelay && (
                <TabsContent value="general" className="space-y-3">
                  <p className="text-sm text-muted-foreground">
                    {t("services.generalRelay.description")}
                  </p>
                  <MultiRelayInput
                    value={state.generalRelay}
                    onChange={(val) =>
                      onChange({ ...state, generalRelay: val })
                    }
                    placeholder="wss://..."
                  />
                </TabsContent>
              ...
              {showSearchRelay && (
                <TabsContent value="search" className="space-y-3">
                  <p className="text-sm text-muted-foreground">
                    {t("services.searchRelay.description")}
                  </p>
                  <MultiRelayInput
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                onChange({ ...state, enableSwap: checked })
              }
            />
          </div>
        </CardHeader>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                onChange({ ...state, enableMessageboardNwc: checked })
              }
            />
          </div>
        </CardHeader>
        {state.enableMessageboardNwc && (
          <CardContent>
            <ServiceCardSelector
              ...
              onChange={(val) =>
                onChange({ ...state, messageboardNwcUrl: val })
              }
      ...
      {validationErrors.length > 0 && (
        <div id="service-config-errors" className="scroll-mt-4">
          <Alert
            variant="destructive"
            className="mt-6 w-full animate-in fade-in slide-in-from-bottom-2"
          >
