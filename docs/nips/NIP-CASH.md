...
- `proof` — REQUIRED unless the slice's current identity is `bearer`. A kind-35521 event, MUST authenticate
  ...
  replayable against a different one, and a proof MUST NOT be replayable to
...
correctly ignores it if present. Type `4` is reserved and MUST NOT be assigned a new meaning — a decoder
ignores it as an unrecognized type on any token that carries it, per the general rule above. (A future
revision MAY add a `min_transfer_mloki` hint type following the same convention; this document doesn't
define one, since it's a best-effort hint an implementation MAY choose to surface via `list_recipients`
instead.)
...
earlier in the request's own processing.** The same TOCTOU class of bug that could let a racing `cash_redeem`
