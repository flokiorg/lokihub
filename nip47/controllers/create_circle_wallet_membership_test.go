...
func TestHandleCreateCircleWalletEvent_Membership_SecondAttemptWhileActive_Rejected(t *testing.T) {
	...
	first := callCreateCircleWallet(t, svc, provider, distinctCircleWalletRequest(t, requesterKey, *provider.WalletPubkey, "first", 100_000, 3600))
	require.Nil(t, first.Error, "first mint for this identity must succeed")

	second := callCreateCircleWallet(t, svc, provider, distinctCircleWalletRequest(t, requesterKey, *provider.WalletPubkey, "second", 100_000, 3600))
...
func TestHandleCreateCircleWalletEvent_Membership_RepeatedAttempts_NeverExceedsPerWalletCap(t *testing.T) {
	...
	var successes int
	for i := 0; i < 5; i++ {
		req := distinctCircleWalletRequest(t, requesterKey, *provider.WalletPubkey, string(rune('a'+i)), perWalletMaxMloki, 3600)
...
// the outcome.
func distinctCircleWalletRequest(t *testing.T, requesterPrivkey, hubWalletPubkey, disambiguator string, maxAmountMloki uint64, expirationSecs int) string {
	t.Helper()
...
func TestHandleCreateCircleWalletEvent_Membership_ConcurrentRace_ExactlyOneWinner(t *testing.T) {
	...
	for i := 0; i < goroutines; i++ {
		...
		reqJSON := distinctCircleWalletRequest(t, requesterKey, *provider.WalletPubkey, string(rune('a'+i)), 50_000, 3600)
		req := &models.Request{}
