...
func TestHandlePayInvoiceEvent_CashWallet_FullBalanceAllowed(t *testing.T) {
	ctx := context.TODO()
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()

	// Create a Cash wallet (child of a hub) with exactly the invoice amount as balance.
	cashWallet, _, err := svc.AppsService.CreateApp("cash-w", "", 0, "never", nil,
		[]string{constants.PAY_INVOICE_SCOPE}, db.AppKindCashWallet, nil, db.ParentKindCash, nil)
	require.NoError(t, err)
	...
	svc.DB.Create(&db.Transaction{
		...
		PaymentHash: "cashfund",
	})

	appPermission := &db.AppPermission{AppId: cashWallet.ID, Scope: constants.PAY_INVOICE_SCOPE}
	...
	var publishedResponse *models.Response
	NewTestNip47Controller(svc).HandlePayInvoiceEvent(ctx, nip47Request, dbRequestEvent.ID, cashWallet, func(r *models.Response, _ nostr.Tags) {
		publishedResponse = r
	}, nostr.Tags{})

	// Payment succeeds (mock LN always succeeds) — no ERROR_RESTRICTED.
	assert.Nil(t, publishedResponse.Error, "full-balance Cash wallet payment must succeed")
}

func TestHandlePayInvoiceEvent_CashWallet_PartialAmountRejected(t *testing.T) {
	ctx := context.TODO()
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()
	...
	cashWallet, _, err := svc.AppsService.CreateApp("cash-w2", "", 0, "never", nil,
		[]string{constants.PAY_INVOICE_SCOPE}, db.AppKindCashWallet, nil, db.ParentKindCash, nil)
	require.NoError(t, err)
	...
	svc.DB.Create(&db.Transaction{
		...
		PaymentHash: "cashfund2",
	})

	svc.DB.Create(&db.AppPermission{AppId: cashWallet.ID, Scope: constants.PAY_INVOICE_SCOPE})
	...
	assert.Equal(t, constants.ERROR_RESTRICTED, publishedResponse.Error.Code, "partial Cash payment must be rejected with ERROR_RESTRICTED")
}

func TestHandlePayInvoiceEvent_NonCash_NoFullBalanceRequirement(t *testing.T) {
	ctx := context.TODO()
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()
...
// TestHandlePayInvoiceEvent_CashWallet_InternalTransferBypassRejected verifies that
// a user-supplied "internal_transfer":true in pay_invoice metadata cannot bypass the
// Cash full-drain enforcement. This is an H1 security test.
func TestHandlePayInvoiceEvent_CashWallet_InternalTransferBypassRejected(t *testing.T) {
	ctx := context.TODO()
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()
	...
	cashWallet, _, err := svc.AppsService.CreateApp("cash-bypass", "", 0, "never", nil,
		[]string{constants.PAY_INVOICE_SCOPE}, db.AppKindCashWallet, nil, db.ParentKindCash, nil)
	require.NoError(t, err)
	svc.DB.Create(&db.Transaction{
		AppId:       &cashWallet.ID,
	...
	assert.NotNil(t, publishedResponse.Error, "Cash partial spend must be rejected even with user-injected internal_transfer flag")
	assert.Equal(t, constants.ERROR_RESTRICTED, publishedResponse.Error.Code,
		"user-injected internal_transfer must not bypass Cash full-drain enforcement")
}

// TestHandlePayInvoiceEvent_JitClaimSliceMetadataSpoofRejected verifies that a
// user-supplied "cash_claim_slice":true in pay_invoice metadata cannot shave the
// fee-reserve headroom off the isolated-balance/budget checks in validateCanPay.
// That flag is meant to be set only by cash_redeem_controller.go itself (for a
// shared Cash wallet's own proof-gated payout, which independently pins the
...
func TestHandlePayInvoiceEvent_JitClaimSliceMetadataSpoofRejected(t *testing.T) {
	...
	require.NotNil(t, publishedResponse.Error, "payment must still require fee-reserve headroom despite spoofed cash_claim_slice")
	assert.Equal(t, constants.ERROR_INSUFFICIENT_BALANCE, publishedResponse.Error.Code,
		"user-injected cash_claim_slice must not bypass the fee-reserve balance check")
}
