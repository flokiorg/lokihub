...
func TestConsolidate_NewIdentityMustBeRecognizedType(t *testing.T) {
	...
	assert.Contains(t, resp.Error.Message, db.CashIdentityCash)
}

// TestConsolidate_CashTarget_MissingIdentityValue_Rejected,
// TestConsolidate_CashTarget_MalformedIdentityValue_Rejected, and
// TestConsolidate_CashTarget_IAPubkeySupplied_Rejected mirror
// cash_transfer's own three cash-mode-target validation tests exactly (same
// three rejections, new call site) — see cash_transfer_controller_test.go.

func TestConsolidate_CashTarget_MissingIdentityValue_Rejected(t *testing.T) {
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()
	...
	require.NotNil(t, resp.Error)
	assert.Equal(t, constants.ERROR_BAD_REQUEST, resp.Error.Code)
	assert.Contains(t, resp.Error.Message, "required for a cash-mode target")
}

func TestConsolidate_CashTarget_MalformedIdentityValue_Rejected(t *testing.T) {
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()
	...
	resp := handleCashConsolidateFor(t, svc, NewTestNip47Controller(svc), wallet, cashConsolidateParams{
		Sources:     []consolidateSourceParam{{WalletPubkey: "a"}, {WalletPubkey: "b"}},
		NewIdentity: cashTransferNewIdentityParam{IdentityType: db.CashIdentityCash, IdentityValue: "not-hex-and-not-64-chars"},
	})
	require.NotNil(t, resp.Error)
	assert.Equal(t, constants.ERROR_BAD_REQUEST, resp.Error.Code)
...
func TestConsolidate_CashTarget_IAPubkeySupplied_Rejected(t *testing.T) {
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()
...
// TestConsolidate_RejectsCashSource is a regression test: a cash-mode source's secret has no signature and
// no binding to the request carrying it — unlike cash_transfer/cash_redeem
// (which never name a foreign wallet_pubkey, so a cash secret only ever
...
// cash-mode source's secret were accepted here, it would sit in plaintext inside
...
// cash note, could decrypt it and race to steal the note. Rejected outright.
func TestConsolidate_RejectsCashSource(t *testing.T) {
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()
	...
	resp := handleCashConsolidateFor(t, svc, NewTestNip47Controller(svc), wallet, cashConsolidateParams{
		Sources: []consolidateSourceParam{
			{WalletPubkey: *wallet.WalletPubkey, CashSecret: "00"},
			{WalletPubkey: "b", CashSecret: "01"},
		},
		NewIdentity: cashTransferNewIdentityParam{IdentityType: db.CashIdentityPubkey, IdentityValue: newPk},
	})
	require.NotNil(t, resp.Error)
	assert.Equal(t, constants.ERROR_BAD_REQUEST, resp.Error.Code)
	assert.Contains(t, resp.Error.Message, "cash-mode sources are not supported")
}
...
// TestHandleCashConsolidateEvent_CashTarget_HappyPath: sum of two pubkey
// sources lands in a new cash-mode wallet, delivered in the clear
// (not nested-encrypted — a cash-mode commitment has no real pubkey to ECDH
// against), and the response never carries the raw secret the caller
// generated locally.
func TestHandleCashConsolidateEvent_CashTarget_HappyPath(t *testing.T) {
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()
	...
	secretHex, secretHash, err := cashwallet.GenerateCashSecret()
	require.NoError(t, err)
	...
	proof1 := buildTransferProofEvent(t, ownerPriv, *s1.WalletPubkey, db.CashIdentityCash, secretHash, "", amount, nil, time.Now())
	proof2 := buildTransferProofEvent(t, ownerPriv, *s2.WalletPubkey, db.CashIdentityCash, secretHash, "", 1_000, nil, time.Now())
	...
	resp := handleCashConsolidateFor(t, svc, NewTestNip47Controller(svc), caller, cashConsolidateParams{
		...
		NewIdentity: cashTransferNewIdentityParam{IdentityType: db.CashIdentityCash, IdentityValue: secretHash},
	})
	...
	assert.True(t, strings.HasPrefix(result.NewWalletToken, "lokicash1"),
		"cash-mode target must be delivered in the clear, not nested-encrypted: got %q", result.NewWalletToken)
	...
	claim := cashWalletClaimByIdentity(t, svc, mergedApp.ID, db.CashIdentityCash, secretHash)
...
func TestHandleCashConsolidateEvent_ConnectionKeyTarget_HappyPath(t *testing.T) {
	...
	// amount matches tests.MockInvoice's own fixed encoded bolt11 amount —
	// see the identical comment in the cash-mode-target test above.
	const amount = uint64(123_000)
	s1 := guardSource(t, svc, hub, ownerPub, amount, 0, 0)
	tests.FundApp(svc, s1.ID, 200_000, "s1-fund")
	s2 := guardSource(t, svc, hub, ownerPub, 1_000, 0, 0)
...
func TestHandleCashConsolidateEvent_ConnectionKeySource_HappyPath(t *testing.T) {
	...
	// (see the identical comment on the cash-mode-target happy path above).
	ownerPriv := nostr.GeneratePrivateKey()
	ownerPub, _ := nostr.GetPublicKey(ownerPriv)
...
func TestConsolidate_ConnectionKeySource_TrustCheckedOncePerCall_NotPerSource(t *testing.T) {
	...
	// on the cash-mode-target happy path above for why arbitrary amounts don't
