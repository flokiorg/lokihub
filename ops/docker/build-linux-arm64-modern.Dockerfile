# -----------------------------------------------------------------------------
# Linux ARM64 Builder (Modern - Debian 12 / Ubuntu 24.04+)
# -----------------------------------------------------------------------------
...
RUN mkdir -p /sysroot/arm64 && \
    debootstrap --variant=minbase --arch=arm64 --foreign \
    --include=libgtk-3-dev,libwebkit2gtk-4.1-dev \
    bookworm /sysroot/arm64 http://deb.debian.org/debian

# -----------------------------------------------------------------------------
# Stage 2: Final Builder (ARM64 Cross-Compile)
# -----------------------------------------------------------------------------
...
# Copy ARM64 Sysroot (from debootstrap)
COPY --from=sysroot-gen /sysroot/arm64/usr/lib/aarch64-linux-gnu /sysroot/arm64/usr/lib/aarch64-linux-gnu
COPY --from=sysroot-gen /sysroot/arm64/usr/include /sysroot/arm64/usr/include
# COPY --from=sysroot-gen /sysroot/arm64/usr/share/pkgconfig /sysroot/arm64/usr/share/pkgconfig
COPY --from=sysroot-gen /sysroot/arm64/lib/aarch64-linux-gnu /sysroot/arm64/lib/aarch64-linux-gnu
COPY --from=sysroot-gen /sysroot/arm64/lib/ld-linux-aarch64.so.1 /sysroot/arm64/lib/ld-linux-aarch64.so.1
...
# Install Zig (for CGO cross-compilation reference if needed)
RUN cd /tmp && \
