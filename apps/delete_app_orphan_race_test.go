...
// DeleteApp's circle_hub/cash_hub child-count guard used to be a
// non-transactional check-then-act (a separate Count() then a separate
// Delete()), so a circle_wallet/cash_wallet creation that committed in the gap
...
func TestCreateApp_ParentNotFound_Rejected(t *testing.T) {
	...
	child, _, err := svc.AppsService.CreateApp(
		...
		[]string{constants.GET_BALANCE_SCOPE}, db.AppKindCashWallet,
		&bogusParentID, db.ParentKindCash, nil,
	)
	require.Error(t, err)
...
func TestCreateApp_ParentWrongKind_Rejected(t *testing.T) {
	...
	// A plain isolated app is not a cash_hub, so it must not be usable as a
	// cash_wallet's parent even though it's a real, existing app row.
	...
	child, _, err := svc.AppsService.CreateApp(
		"child", "", 0, constants.BUDGET_RENEWAL_NEVER, nil,
		[]string{constants.GET_BALANCE_SCOPE}, db.AppKindCashWallet,
		&notAHub.ID, db.ParentKindCash, nil,
	)
	require.Error(t, err)
	assert.ErrorIs(t, err, constants.ErrInvalidParams)
	assert.Nil(t, child)
...
// test for the orphan race itself: race a cash_wallet creation under hub
...
func TestDeleteAppAndCreateChild_ConcurrentRace_NeverOrphans(t *testing.T) {
	...
	for trial := 0; trial < trials; trial++ {
		...
		require.NoError(t, svc.DB.Model(&db.App{}).
			Where("parent_app_id = ? AND parent_kind = ?", hub.ID, db.ParentKindCash).
		...
		if hubStillExists {
			var children []db.App
			svc.DB.Where("parent_app_id = ? AND parent_kind = ?", hub.ID, db.ParentKindCash).Find(&children)
			for _, c := range children {
				require.NoError(t, svc.DB.Delete(&c).Error)
			}
			require.NoError(t, svc.DB.Delete(hub).Error)
		} else if orphanCount > 0 {
			require.NoError(t, svc.DB.Where("parent_app_id = ? AND parent_kind = ?", hub.ID, db.ParentKindCash).Delete(&db.App{}).Error)
		}
	}
}
