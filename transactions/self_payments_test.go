...
func TestSendPaymentSync_SelfPayment_IsolatedAppToNoApp(t *testing.T) {
	...
	// give the isolated app 123 loki (fee reserve not applied for self payments)
	svc.DB.Create(&db.Transaction{
		AppId:       &app.ID,
...
func TestSendPaymentSync_SelfPayment_IsolatedAppToApp_AmountProvidedIgnoredOnNonZeroAmountInvoice(t *testing.T) {
	...
	amountMloki := uint64(1000)
	transaction, err := transactionsService.SendPaymentSync(tests.MockInvoice, &amountMloki, nil, svc.LNClient, &app.ID, &dbRequestEvent.ID)
