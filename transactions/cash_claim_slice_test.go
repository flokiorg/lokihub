...
// Shared Cash wallets (see cashwallet/create.go, nip47/controllers/cash_redeem_controller.go)
// let several recipients each claim their own slice of ONE wallet's isolated
// balance. enforceCashFullDrain's whole-wallet-balance check is wrong for that
// case — it would reject a recipient's payout whenever OTHER recipients'
// unclaimed slices are still sitting in the same balance. cash_redeem
// exempts itself via metadata["cash_claim_slice"] = true (mirroring the
...
// SendPaymentSync.
//
// TestSendPaymentSync_CashWallet_ClaimSlice_BypassesWholeWalletFullDrainCheck
...
// TestSendPaymentSync_CashWallet_ClaimSlice_OtherSlicesUnaffected goes further:
...
// newSharedCashWallet creates a cash_wallet child (bypassing cashwallet.Create,
// per this package's own established pattern in cash_internal_transfer_test.go)
// granted CASH_REDEEM_SCOPE (the scope a real shared Cash wallet carries),
...
// check.
func newSharedCashWallet(t *testing.T, svc *tests.TestService, budgetCapLoki int) *db.App {
	t.Helper()
...
// TestSendPaymentSync_CashWallet_ClaimSlice_ExactBalance_NoFeeReserveHeadroomNeeded
...
// being claimed — which is exactly how a shared Cash wallet's last (or only)
// recipient's slice works (cashwallet.Commit funds the wallet with the exact
// sum of declared recipient amounts, and cash_redeem requires the invoice
// amount to exactly equal the claimed slice). Without skipFeeReserve, no
// real (non-self) cash_redeem payout could ever succeed, regardless of
// amount.
func TestSendPaymentSync_CashWallet_ClaimSlice_ExactBalance_NoFeeReserveHeadroomNeeded(t *testing.T) {
	...
	wallet := newSharedCashWallet(t, svc, 123)
	...
	_, err = transactionsService.SendPaymentSync(
		tests.MockInvoice, nil, map[string]interface{}{"cash_claim_slice": true},
		svc.LNClient, &wallet.ID, &dbRequestEvent.ID,
	)
	assert.NoError(t, err, "a wallet funded with exactly the claimed amount must still be payable via cash_redeem — no fee-reserve headroom should be required")
...
func TestSendPaymentSync_CashWallet_ClaimSlice_BypassesWholeWalletFullDrainCheck(t *testing.T) {
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()
	...
	// mloki, far more than the fee-reserve floor, which enforceCashFullDrain
	// would normally reject.
	wallet := newSharedCashWallet(t, svc, 500)
	svc.DB.Create(&db.Transaction{
		AppId: &wallet.ID, State: constants.TRANSACTION_STATE_SETTLED,
	...
	assert.Error(t, err, "without the cash_claim_slice exemption, a partial-of-shared-balance payment must still be rejected as a partial spend")
	...
	assert.NoError(t, err, "cash_claim_slice=true must bypass the whole-wallet full-drain check")
}

func TestSendPaymentSync_CashWallet_ClaimSlice_OtherSlicesUnaffected(t *testing.T) {
	svc, err := tests.CreateTestService(t)
	require.NoError(t, err)
	defer svc.Remove()
	...
	wallet := newSharedCashWallet(t, svc, 600)
	svc.DB.Create(&db.Transaction{
		AppId: &wallet.ID, State: constants.TRANSACTION_STATE_SETTLED,
	...
	require.NoError(t, svc.DB.Create(dbRequestEvent2).Error)
	_, err = transactionsService.SendPaymentSync(
		tests.MockZeroAmountInvoice, ptrUint64Transactions(50_000), map[string]interface{}{"cash_claim_slice": true},
		svc.LNClient, &wallet.ID, &dbRequestEvent2.ID,
	)
