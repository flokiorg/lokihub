...
func TestMakeInvoice_NoApp(t *testing.T) {
	...
	transaction, err := transactionsService.MakeInvoice(ctx, 1234, "Hello world", "", 0, txMetadata, svc.LNClient, nil, nil, nil, nil, nil, nil, nil)
...
func TestMakeInvoice_MetadataTooLarge(t *testing.T) {
	...
	transactionsService := NewTransactionsService(svc.DB, svc.EventPublisher)
	transaction, err := transactionsService.MakeInvoice(ctx, 1234, "Hello world", "", 0, metadata, svc.LNClient, nil, nil, nil, nil, nil, nil, nil)
...
func TestMakeInvoice_App(t *testing.T) {
	...
	transactionsService := NewTransactionsService(svc.DB, svc.EventPublisher)
	transaction, err := transactionsService.MakeInvoice(ctx, 1234, "Hello world", "", 0, nil, svc.LNClient, &app.ID, &dbRequestEvent.ID, nil, nil, nil, nil, nil)
